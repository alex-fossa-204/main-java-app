...
public class ReportServiceTest {
	...
	public void testAddUserReport() {
		...
		try {
			...
			if (result) {
				System.out.println("Report Successfully Added: " + username + " " + reportTitle + " " + reportBody + " "
						+ laborCost);
			}
			if (!result) {
	...
	public void testGetAllUsersReports() throws ParseException {
		...
		try {
			Map<UserDTO, List<ReportDTO>> usersReports = reportService.getAllUsersReportsDtoMapByDate(stringDate);
			...
			System.out.println("Json: " + parseMapToJson(usersReports));
			System.out.println("\n");
		} catch (ServiceException e) {
			System.err.println(e.getMessage());
		}
	...
	@Test
	public void testGetingleUserReports() throws ParseException {
		String currentUserName = "dogeadmin16";
		IReportService reportService = new ReportService();
		System.out.println("Get user report list:" + currentUserName);
		try {
			List<Report> userReportsList = reportService.getAllReportsForSingleUser(currentUserName);
			userReportsList.stream().forEach(report -> System.out.println(report));
		} catch (ServiceException e) {
			System.err.println(e.getMessage());
		}
		System.out.println("\n");
