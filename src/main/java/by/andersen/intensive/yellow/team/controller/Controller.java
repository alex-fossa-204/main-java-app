package by.andersen.intensive.yellow.team.controller;
...
import by.andersen.intensive.yellow.team.controller.command.manager.CommandManager;
import by.andersen.intensive.yellow.team.controller.page.Page;
import by.andersen.intensive.yellow.team.utils.daemon.TimerDaemon;
...
//		timerDaemon.start();
	...
	private void redirect(Page page, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
			throws IOException {
		String redirectUrl = page.getPageUrl();
		httpServletResponse.sendRedirect(redirectUrl);
	}

	private void forward(Page page, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
			throws ServletException, IOException {
		...
		if (!NONE_MESSAGE.getMessage().equalsIgnoreCase(pageMessage)) {
			String message = MessageEnum.valueOf(pageMessage).getMessage()
					.replace(MessageEnum._MESSAGE.getMessage(), "")
					.concat(MessageEnum.PRESS_HERE_TO_CONTINUE.getMessage());
